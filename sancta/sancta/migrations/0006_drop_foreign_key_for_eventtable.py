...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if settings.IS_TESTING:
            return
        result = dbs['sancta_db'].execute("\
            SELECT * FROM  information_schema.key_column_usage\
            WHERE constraint_schema='{0}' AND\
            table_name = '{1}' AND constraint_name = '{2}'".format(
                settings.DATABASES['sancta_db']['NAME'],
                'mf_calendar_event',
                'FK_calendarEventId_systemObjectId'
            )
        )
        if bool(result):
            dbs['sancta_db'].execute(
                "ALTER TABLE `mf_calendar_event` DROP\
                FOREIGN KEY `FK_calendarEventId_systemObjectId`;"
            )
